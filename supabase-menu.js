...
const FOTOS_PIZZA = {
  ...
  'Cuatro Quesos': 'cuatro_quesos.png',
  'Seoul': 'kr_seoul.png',
  'Mongoliana': 'mongoliana.png',
  'Chifa': 'chifa.png',
};
...
function tarjetaPizzaHTML(item, esFusion) {
  ...
  const foto = FOTOS_PIZZA[nombreLimpio];

  return `
    <article class="pizza-card ${esFusion ? 'fusion-card' : ''}">
      <div class="pizza-photo" data-short="${nombreLimpio}">
        ${foto ? `<img src="img/${foto}" alt="${nombreLimpio}" loading="lazy">` : ''}
      </div>
